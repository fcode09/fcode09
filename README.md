<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=61AFEF&height=200&section=header&text=Freddy%20Quea&fontSize=50&fontColor=282C34&animation=fadeIn&fontAlignY=35&desc=Full-Stack%20Developer%20%7C%20Solution%20Architect&descAlignY=55&descSize=20" />

<br>

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=22&duration=3000&pause=1000&color=61AFEF&center=true&vCenter=true&width=500&lines=Solution+Architect;Full-Stack+Developer;AI+Agent+Systems+Specialist;Cloud+Native+Engineer;Open+Source+Contributor)](https://github.com/fq-09)

<br>

<p>
  <a href="https://www.linkedin.com/in/freddy-quea/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/></a>
  <a href="https://github.com/fq-09"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/></a>
  <a href="https://www.instagram.com/coders.qj/"><img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" alt="Instagram"/></a>
  <a href="mailto:ruben.quea@tecsup.edu.pe"><img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/></a>
</p>

</div>

<img src="https://capsule-render.vercel.app/api?type=soft&color=61AFEF&height=3&section=header" width="100%">

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Rocket.png" width="35" /> About Me

<img align="right" src="https://raw.githubusercontent.com/abhisheknaiidu/abhisheknaiidu/master/code.gif" width="380" />

Innovative **Solution Architect** and **Full-Stack Developer** specializing in AI agent systems, GPS-based solutions, and cloud-native architectures. I design and build intelligent routing systems, automation pipelines, and cross-platform applications that solve real-world problems.

```typescript
const freddy = {
  code: ['TypeScript', 'Go', 'Python', 'Dart', 'JavaScript'],
  mobile: ['React Native', 'Flutter'],
  frontend: ['Angular', 'React', 'Astro', 'Next.js'],
  backend: ['NestJS', 'Node.js', 'Go', 'Python'],
  cloud: ['Google Cloud Platform', 'Docker', 'Kubernetes'],
  databases: ['PostgreSQL', 'MongoDB', 'Prisma', 'Mongoose', 'Redis'],
  currentFocus: 'AI agent routing systems & GPS optimization',
  challenge: 'Building intelligent automation with LLM agents',
  passions: ['Agent Systems', 'Clean Architecture', 'Performance Optimization']
};
```

- 🚀 Architecting **GPS-based routing systems** with real-time optimization
- 🤖 Developing **AI agent routing** and **LLM-powered automation** workflows
- 📱 Building **cross-platform mobile apps** with Flutter & React Native
- ☁️ Managing **Google Cloud** infrastructure and serverless deployments
- 🐹 Writing **Go** microservices for high-performance backend systems

<br clear="right"/>

<div align="center">
  <a href="https://github.com/fq-09?tab=repositories"><img src="https://img.shields.io/badge/View_My_Work-61AFEF?style=for-the-badge&logoColor=white" alt="View My Work"/></a>
</div>

<img src="https://capsule-render.vercel.app/api?type=soft&color=61AFEF&height=3&section=header" width="100%">

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Hammer%20and%20Wrench.png" width="30" /> Tech Stack

### Languages
<p align="center">
  <img src="https://skillicons.dev/icons?i=go,ts,js,python,dart&theme=dark" />
</p>

### Frontend & Mobile
<p align="center">
  <img src="https://skillicons.dev/icons?i=angular,react,astro,nextjs,flutter&theme=dark" />
</p>

### Backend & APIs
<p align="center">
  <img src="https://skillicons.dev/icons?i=nestjs,nodejs,express,graphql&theme=dark" />
</p>

### Databases & ORMs
<p align="center">
  <img src="https://skillicons.dev/icons?i=postgres,mongodb,redis,prisma&theme=dark" />
</p>

### Cloud & DevOps
<p align="center">
  <img src="https://skillicons.dev/icons?i=gcp,docker,kubernetes,git,githubactions&theme=dark" />
</p>

### Tools & Design
<p align="center">
  <img src="https://skillicons.dev/icons?i=figma,vscode,postman,bash,linux&theme=dark" />
</p>

<br>

<details>
<summary><b>🔍 Complete Tech Stack</b></summary>

| Category | Technologies |
|----------|-------------|
| **Languages** | Go, TypeScript, JavaScript, Python, Dart |
| **Frontend** | Angular, React, Astro, Next.js, Tailwind CSS, Sass |
| **Mobile** | React Native, Flutter |
| **Backend** | NestJS, Node.js, Express, Go, GraphQL, REST APIs |
| **Databases** | PostgreSQL, MongoDB, Redis, Prisma ORM, Mongoose |
| **Cloud** | Google Cloud Platform (GCP), Cloud Functions, Cloud Run |
| **DevOps** | Docker, Kubernetes, GitHub Actions, CI/CD |
| **AI/ML** | OpenAI API, LangChain, Vector Databases |
| **Tools** | Figma, VS Code, Postman, Git, Linux |

</details>

<img src="https://capsule-render.vercel.app/api?type=soft&color=61AFEF&height=3&section=header" width="100%">

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Bar%20Chart.png" width="35" /> GitHub Analytics

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=fq-09&show_icons=true&theme=onedark&hide_border=true&bg_color=282C34&title_color=61AFEF&icon_color=61AFEF&text_color=E5C07B" height="180" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=fq-09&theme=onedark&hide_border=true&background=282C34&stroke=61AFEF&ring=61AFEF&fire=E06C75&currStreakLabel=61AFEF&sideLabels=E5C07B" height="180" />
</div>

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=fq-09&layout=compact&theme=onedark&hide_border=true&bg_color=282C34&title_color=61AFEF&text_color=E5C07B&langs_count=8" height="180" />
</div>

<div align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=fq-09&theme=onedark&no-frame=true&column=7&margin-w=10&rank=SSS,SS,S,AAA,AA,A,B,C&bg_color=282C34" />
</div>

<br>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/fq-09/fq-09/output/github-contribution-grid-snake-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/fq-09/fq-09/output/github-contribution-grid-snake.svg">
  <img alt="github contribution grid snake animation" src="https://raw.githubusercontent.com/fq-09/fq-09/output/github-contribution-grid-snake-dark.svg">
</picture>

<img src="https://capsule-render.vercel.app/api?type=soft&color=61AFEF&height=3&section=header" width="100%">

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Bookmark.png" width="35" /> Featured Projects

<div align="center">

<!-- Add your featured projects here - replace repo names -->
<a href="https://github.com/fq-09">
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=fq-09&repo=gps-routing-lib&theme=onedark&bg_color=282C34&title_color=61AFEF&icon_color=61AFEF&text_color=E5C07B&hide_border=true" />
</a>

<a href="https://github.com/fq-09">
  <img src="https://github-readme-stats.vercel.app/api/pin/?username=fq-09&repo=ai-agent-router&theme=onedark&bg_color=282C34&title_color=61AFEF&icon_color=61AFEF&text_color=E5C07B&hide_border=true" />
</a>

</div>

<p align="center">
  <em>🚧 Replace with your actual repository names above to showcase your work!</em>
</p>

<img src="https://capsule-render.vercel.app/api?type=soft&color=61AFEF&height=3&section=header" width="100%">

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Hand%20gestures/Waving%20Hand.png" width="35" /> Connect With Me

<div align="center">
  <a href="https://www.linkedin.com/in/freddy-quea/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
  </a>
  <a href="mailto:ruben.quea@tecsup.edu.pe">
    <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/>
  </a>
  <a href="https://github.com/fq-09">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
  </a>
  <a href="https://www.instagram.com/coders.qj/">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" alt="Instagram"/>
  </a>
</div>

<br>

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=fq-09&style=for-the-badge&color=61AFEF&label=PROFILE+VIEWS" alt="Profile Views" />
</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=61AFEF&height=120&section=footer" width="100%">

<div align="center">
  <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Laptop.png" width="35" />
  <br>
  <em><b>Let's build something amazing together!</b></em>
  <br>
  <em>Open for collaborations and interesting projects 🚀</em>
</div>
